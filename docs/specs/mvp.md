...
| 画面 | 役割 |
| --- | --- |
| スタート画面 | サイト説明、モード×時間の選択、ランキング表示 |
| プレイ画面 | タイピング本編。HUD・演出・課題文・仮想キーボード |
...
```mermaid
flowchart TD
  start[スタート画面] -->|モードと時間を選択| play[プレイ画面]
  play -->|制限時間終了| result[リザルト画面]
  play -->|終了ボタン確定| result
...
- プレイ中の「終了」は確認ダイアログを出し、確定した場合のみリザルトへ進む。
...
| 終端 | 文は「。」までを 1 課題とする |
| 入力方式 | ローマ字入力（画面上は日本語原文とローマ字の 2 段表示）。し/shi・si、ふ/fu・hu、じ/ji・zi など一般的な別表記を許可し、表示ローマ字は選んだ経路に合わせて更新する |
...
- 長文モードは、かな（segment）単位で複数のローマ字経路のうちいずれかの次キーと一致すれば正解とする。表示中のローマ字とハイライトは、ユーザーが選んだ経路に合わせて更新する。
- コーディングモードは、表示文字列どおりの 1 文字と一致したキー入力だけを正解とする。
- 正解時、カーソルを 1 文字進める。
...
- リザルト画面で名前を入力して登録する。
- 名前は 1〜12 文字。未入力で登録した場合は「ゲスト」。
- 不適切な文字列の拒否は MVP では最低限（空文字以外は「ゲスト」置換のみ）でよい。
- 同じプレイ結果の二重登録はしない。
...
| 画面中央・上寄り | 課題文（日本語またはコード） |
| 課題文の直下 | ローマ字（長文モード）。コーディングモードでは入力対象のコード列を兼ねてよい |
| 課題文付近 | 出典（「日常会話より」「（参照元）より」「HTMLより」など） |
| 画面中央・下 | 仮想キーボード。実キー押下と連動して光る |
| 画面右・下寄り | コンボ表示 |
...
9. 終了ボタンは確認ダイアログを出す。
10. ランキング登録時に名前入力。未入力は「ゲスト」。1〜12 文字。
